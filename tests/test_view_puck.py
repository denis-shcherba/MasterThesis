...
def run_puck_from_h5(env, view_in_between=False):
    cylinder_pos_list = []
    with h5py.File("table_demo.h5", 'a') as f:
        ...
        for i, demo_name in enumerate(demo_groups):
            ...
            cylinder_pos = f[demo_name]['cylinder'][:] 
            

            original_shape = cylinder_pos.shape
            print(cylinder_pos)
            cylinder_pos_list.append(cylinder_pos)

            env.unwrapped._spawn_cylinder_scene(center=cylinder_pos)
            env.unwrapped.C.view(view_in_between, f"h5 data point {i}")

    for i in range(len(cylinder_pos_list)):    
        env.unwrapped.C.addFrame(f"test_cylinder_{i}").setPosition(cylinder_pos_list[i]).setShape(ry.ST.cylinder, size=[0.03, 0.04]).setColor([0, 0, 0])
